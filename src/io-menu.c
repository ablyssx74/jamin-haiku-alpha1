...
float gCurrentSampleRate = 44100.0f;
int32_t gCurrentBufferSize = 1024; 
//void (*UpdateSampleRate_Ptr)(float rate) = NULL;
void (*UpdateAudioParams_Ptr)(float rate, int32_t bufferSize) = NULL;
...
void io_update_engine_rate(float newRate) {
    if (newRate < 100.0f) return;
    
    if (gCurrentSampleRate != newRate) {
        gCurrentSampleRate = newRate;
        // If JAMin has a function to recalculate its EQ/Filters, 
        // call it here. For example:
        // dsp_init_filters(gCurrentSampleRate); 
    }
}
...
static void
iomenu_set_samplerate(GtkWidget *widget, gpointer data)
{
    float new_rate = (float)GPOINTER_TO_INT(data);
    
    // 1. Update the global so the engine and UI stay in sync
    gCurrentSampleRate = new_rate;
    
    // 2. Call the consolidated bridge with both values
    if (UpdateAudioParams_Ptr) {
        UpdateAudioParams_Ptr(new_rate, gCurrentBufferSize);
    }
    update_status_bar_text();
}

static void
iomenu_set_buffersize(GtkWidget *widget, gpointer data)
{
    int32_t new_size = GPOINTER_TO_INT(data);
    
    // 1. Update the global
    gCurrentBufferSize = new_size;
    
    // 2. Call the consolidated bridge with both values
    if (UpdateAudioParams_Ptr) {
        UpdateAudioParams_Ptr(gCurrentSampleRate, new_size);
    }
    update_status_bar_text();
}
...
{
	gtk_widget_show_all(pulldown_menu); 
...
iomenu_bind(GtkWidget *main_window)
{
    ...
    /* 1. Add Input/Output items as before */
    //in_item = iomenu_add_item(pulldown_menu, gtk_menu_item_new_with_label(_("In")));
    //out_item = iomenu_add_item(pulldown_menu, gtk_menu_item_new_with_label(_("Out")));

    /* 2. Add a Separator */
    iomenu_add_item(pulldown_menu, gtk_separator_menu_item_new());

    /* 3. Add Sample Rate Submenu */
    GtkWidget *sr_item = iomenu_add_item(pulldown_menu, gtk_menu_item_new_with_label(_("Sample Rate")));
    GtkWidget *sr_menu = gtk_menu_new();
    gtk_menu_item_set_submenu(GTK_MENU_ITEM(sr_item), sr_menu);

    int rates[] = {44100, 48000, 88200, 96000, 192000};
    for (int i = 0; i < 5; i++) {
        char label[32];
        sprintf(label, "%d Hz", rates[i]);
        GtkWidget *rate_opt = gtk_menu_item_new_with_label(label);
        g_signal_connect(G_OBJECT(rate_opt), "activate", 
                         G_CALLBACK(iomenu_set_samplerate), GINT_TO_POINTER(rates[i]));
        gtk_menu_shell_append(GTK_MENU_SHELL(sr_menu), rate_opt);
    }

    /* 4. Add Buffer Size Submenu */
    GtkWidget *bs_item = iomenu_add_item(pulldown_menu, gtk_menu_item_new_with_label(_("Buffer Size")));
    GtkWidget *bs_menu = gtk_menu_new();
    gtk_menu_item_set_submenu(GTK_MENU_ITEM(bs_item), bs_menu);

    int sizes[] = {128, 256, 512, 1024, 2048, 4096};
    for (int i = 0; i < 6; i++) {
        char label[32];
        sprintf(label, "%d frames", sizes[i]);
        GtkWidget *size_opt = gtk_menu_item_new_with_label(label);
        g_signal_connect(G_OBJECT(size_opt), "activate", 
                         G_CALLBACK(iomenu_set_buffersize), GINT_TO_POINTER(sizes[i]));
        gtk_menu_shell_append(GTK_MENU_SHELL(bs_menu), size_opt);
    }
...
void update_status_bar_text() {
    ...
    extern GtkWidget *main_window; // Ensure you have access to the main window
    
    // Find the label by its name "status_label"
    GtkWidget *label = lookup_widget(main_window, "status_label");
    
    if (label) {
        char new_text[256];
        sprintf(new_text, "Stopped | some CPU | %d frames | %.0f Hz | Focus - something", 
                gCurrentBufferSize, gCurrentSampleRate);
        gtk_label_set_text(GTK_LABEL(label), new_text);
    } else {
        fprintf(stderr, "[JAMin-UI] Error: Could not find status_label widget!\n");
    }
}
